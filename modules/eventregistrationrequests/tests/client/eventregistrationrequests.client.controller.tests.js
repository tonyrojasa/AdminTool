(function () {
  ...
  describe('Eventregistrationrequests Controller Tests', function () {
    ...
    beforeEach(inject(function ($controller, $rootScope, _$state_, _$httpBackend_, _Authentication_, _EventregistrationsService_) {
      ...
      // Initialize the Eventregistrationrequests controller.
    ...
    describe('vm.save() as create', function () {
      ...
      it('should send a POST request with the form input values and then locate to new object URL', inject(function (EventregistrationsService) {
        // Set POST response
        $httpBackend.expectPOST('api/eventregistrationrequests', sampleEventregistrationPostData).respond(mockEventregistration);
        ...
        expect($state.go).toHaveBeenCalledWith('eventregistrationrequests.view', {
      ...
      it('should set $scope.vm.error if error', function () {
        ...
        $httpBackend.expectPOST('api/eventregistrationrequests', sampleEventregistrationPostData).respond(400, {
    ...
    describe('vm.save() as update', function () {
      ...
      it('should update a valid Eventregistration', inject(function (EventregistrationsService) {
        // Set PUT response
        $httpBackend.expectPUT(/api\/eventregistrationrequests\/([0-9a-fA-F]{24})$/).respond();
      ...
      it('should set $scope.vm.error if error', inject(function (EventregistrationsService) {
        ...
        $httpBackend.expectPUT(/api\/eventregistrationrequests\/([0-9a-fA-F]{24})$/).respond(400, {
          message: errorMessage
        });
    ...
    describe('vm.remove()', function () {
      beforeEach(function () {
        // Setup Eventregistrationrequests
        $scope.vm.eventregistration = mockEventregistration;
      });

      it('should delete the Eventregistration and redirect to Eventregistrationrequests', function () {
        // Return true on confirm message
        spyOn(window, 'confirm').and.returnValue(true);

        $httpBackend.expectDELETE(/api\/eventregistrationrequests\/([0-9a-fA-F]{24})$/).respond(204);

        $scope.vm.remove();
        $httpBackend.flush();

        expect($state.go).toHaveBeenCalledWith('eventregistrationrequests.list');
      });
